...
export function BrowseContainer({ slides }) {
	...
	return profile.displayName ? (
		<>
			...
			<Header
				src={
					background.backdrop_path !== undefined
						? `${SOURCE.BASE_IMG_URL}${background.backdrop_path}`
						: "/images/misc/bg.jpg"
				}
			...
			>
				...
				<Header.Feature>
					<Header.FeatureCallOut>
						Watch {background.title || background.name} Now
					</Header.FeatureCallOut>
						...
						<Card.Entities>
							{slideItem.data.map((item) => (
								<Card.Item key={item.docId || item.id} item={item}>
									<Card.Image
										src={
											`${SOURCE.BASE_IMG_URL}${item.poster_path}` ||
