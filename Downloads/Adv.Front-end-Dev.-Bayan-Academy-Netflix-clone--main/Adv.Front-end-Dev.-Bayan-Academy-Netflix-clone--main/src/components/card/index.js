...
Card.Feature = function CardFeature({ children, category, ...restProps }) {
	...
	const mediaType = is_MediaType_Undedfined
		? category === "series"
			? "tv"
			: "movie"
		: itemFeature.media_type;
	showFeature
		? (itemFeature.rating = useFetchRating(itemFeature.id, mediaType))
		: null;
	return showFeature ? (
		<Feature
			{...restProps}
			src={`${SOURCE.BASE_IMG_URL}${itemFeature.backdrop_path}`}
		>
			<Content>
				<FeatureTitle>{itemFeature.title || itemFeature.name}</FeatureTitle>
				<FeatureText>
					{itemFeature.description || itemFeature.overview}
				</FeatureText>
